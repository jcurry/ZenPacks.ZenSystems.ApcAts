...
# If no data supplied then simply return
        ...
        if not inputtable:
            log.warn(' No SNMP response from %s for the %s plugin  for Input Table' % ( device.id, self.name() ) )
            log.warn( "Data= %s", tabledata )
            return

        inputPhasetable = tabledata.get('ApcAtsInputPhaseTable')
        if not inputPhasetable:
            log.warn(' No SNMP response from %s for the %s plugin  for Input Phase Table' % ( device.id, self.name() ) )
            log.warn( "Data= %s", tabledata )
            return
...
# Decode numeric input type into descriptive string
                ...
                log.warn( ' Error in ApcAtsInputMap modeler plugin %s', errorInfo)
                continue
            rm.append(om)
#            log.info('rm %s' % (rm) )
